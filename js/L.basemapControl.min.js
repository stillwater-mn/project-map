L.BasemapControl = L.Control.extend({
  options: {
    position: 'bottomleft',
    layers: []
  },

  onAdd: function (map) {
    this._map = map;
    this._currentIndex = 0;

    // Container
    const container = L.DomUtil.create('div', 'leaflet-control-basemap');
    this._container = container;

    // Preview image container
    this._img = L.DomUtil.create('div', 'basemap-img', container);

    L.DomEvent.disableClickPropagation(container);

    // Mini preview map (hidden)
    this._switchMap = L.map(container, {
      zoomControl: false,
      attributionControl: false,
      minZoom: map.getMinZoom() - 3,
      maxZoom: map.getMaxZoom() - 3,
      crs: map.options.crs,
      dragging: false,
      touchZoom: false,
      doubleClickZoom: false,
      scrollWheelZoom: false
    });

    // Tile layer used for preview loading
    this._switchLayer = L.tileLayer(
      this.options.layers[this._currentIndex].layer._url,
      {
        attribution:
          this.options.layers[this._currentIndex].layer.options.attribution
      }
    ).addTo(this._switchMap);

    // Set initial basemap
    this._setLayer(this.options.layers[this._currentIndex].layer);
    this._updateOutline();

    // Events
    L.DomEvent.on(this._img, 'click', this._nextLayer, this);
    map.on('moveend zoomend', this._updatePreview, this);

    this._initializePreview();

    return container;
  },

  _initializePreview: function () {
    setTimeout(() => this._updatePreview(), 100);
  },

  _setLayer: function (layer) {
    if (this._currentLayer) {
      this._map.removeLayer(this._currentLayer);
    }
    this._currentLayer = layer.addTo(this._map);
  },

  _nextLayer: function () {
    this._currentIndex =
      (this._currentIndex + 1) % this.options.layers.length;

    this._setLayer(this.options.layers[this._currentIndex].layer);
    this._updatePreview();
    this._updateOutline();

    this._map.fire('basemapChange', {
      layer: this.options.layers[this._currentIndex].layer
    });
  },

  _updatePreview: function () {
    const center = this._map.getCenter();
    const zoom = this._map.getZoom();
    const previewZoom = Math.max(zoom - 3, 0);

    this._switchMap.setView(center, previewZoom);

    const nextIndex =
      (this._currentIndex + 1) % this.options.layers.length;
    const nextLayer = this.options.layers[nextIndex].layer;

    const tileUrl = nextLayer._url.replace('{s}', 'a');

    this._img.classList.add('loading');
    this._switchLayer.setUrl(tileUrl);

    this._switchLayer.once('load', () => {
      this._img.style.backgroundImage = `url(${tileUrl})`;
      this._img.classList.remove('loading');
    });

    this._switchLayer.once('error', () => {
      this._img.classList.add('loading');
    });
  },

  _updateOutline: function () {
    const current = this.options.layers[this._currentIndex];

    this._container.classList.remove(
      'vector-active',
      'satellite-active'
    );

    if (!current || !current.name) return;

    if (current.name.toLowerCase() === 'satellite') {
      this._container.classList.add('satellite-active');
    } else {
      this._container.classList.add('vector-active');
    }
  },

  changeBasemap: function (index) {
    if (index >= 0 && index < this.options.layers.length) {
      this._currentIndex = index;
      this._setLayer(this.options.layers[index].layer);
      this._updatePreview();
      this._updateOutline();
    }
  }
});

L.basemapControl = function (options) {
  return new L.BasemapControl(options);
};
